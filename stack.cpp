...
Stack<T>::Stack()
{
  int start = (int) DEFAULTCAPACITY;
  //std::cout<<"The initial value is "<<start<<std::endl;
  items = new T[start];
  max_items = start;
  num_items = 0;
}
...
void Stack<T>::Push(const T& item) {
  ...
  //std::cout<<"Pushing in "<<item<<std::endl;
  if(num_items == max_items){
    //std::cout<<"Number of items is "<<num_items<<" and max items is "<<max_items<<std::endl;
    //std::cout<<"The Resize is  "<<EXPANSIONFACTOR*max_items<<std::endl;
    Resize(EXPANSIONFACTOR*max_items);
    //std::cout<<"The Resize is changed it to "<<max_items<<std::endl;
  }
  //std::cout<<"Number of items is "<<num_items<<" and max items is "<<max_items<<std::endl;
...
T Stack<T>::Pop() {
  // complete your implementation below
  T it = items[num_items-1];
  //std::cout<<"Popping out "<<it<<std::endl;
  num_items-=1;
  ...
  if(max_items / EXPANSIONFACTOR< DEFAULTCAPACITY){
    ...
    n=max_items / EXPANSIONFACTOR;
  }

  if((1.0 / SHRINKRATE)*max_items > num_items){
    Resize(n);
  }
  //std::cout<<"Popped item  "<<it<<" and num_items is "<<num_items<<std::endl;
  return it;
};
...
T Stack<T>::Peek() {
  // complete your implementation below
  //std::cout<<"The value of the top is "<<items[num_items-1]<<std::endl;
  if(num_items <= 0){
    return NULL;
  }
  T it = items[num_items-1];
...
void Stack<T>::Resize(size_t n) {
  // complete your implementation below
  //std::cout<<"Max items is "<<max_items<<std::endl;
  max_items =n;
  //std::cout<<"Max items is "<<max_items<<std::endl;
   T* it = new T[max_items];
