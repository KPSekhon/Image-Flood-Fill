...
class CensorColorPicker : public ColorPicker
{
  ...
  private:
    ...
    void mosiacImageMaker();
    unsigned int blockAdjusterWidth(unsigned int b); 
    unsigned int blockAdjusterHeight(unsigned int b); 
    void blockMaker(unsigned int x, unsigned int y ,unsigned int maxX, unsigned int maxY);
    HSLAPixel averageColorFinder(unsigned int x, unsigned int y,unsigned int maxX, unsigned int maxY);
